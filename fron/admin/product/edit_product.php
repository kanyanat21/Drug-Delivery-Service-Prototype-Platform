...
if(isset($_GET['med_id']) && !empty($_GET['med_id'])){
    $med_id = $_GET['med_id'];
    $sql = "SELECT * FROM tb_product WHERE med_id = '$med_id'";
        ...
        if(in_array($ext,$extension)){
            if(!file_exists($target.$filename)){
                if(move_uploaded_file($filetmp,$target.$filename)){
                    ...
                    $alert .= 'window.location.href = "product.php";';
                    $alert .= '</script>';
                    echo $alert;
                    exit();
                }
                ...
                if(move_uploaded_file($filemp,$target.newfilename)){
...
<body class="app">   	
    ...
    <div class="app-wrapper">
	    <div class="app-content pt-3 p-md-3 p-lg-4">
			...
			<h3 class="section-title">แก้ไขข้อมูลสินค้า</h3>
			    <hr class="mb-4">
                <div class="row g-4 settings-section">
	                <div class="col-12 col-md-4">
                        <br>
		                <a href="product.php" class="btn btn-secondary text-white mb-4 float-right">ย้อนกลับ</a>
                        <br>
                        <br>
		                <div class="app-card app-card-settings shadow-sm p-4">   
						    <div class="app-card-body">
							    ...
							    <form action="update_product.php" method="POST" enctype= multipart/form-data>
									<input type="hidden" value="<?php echo $row["med_id"]; ?>" name=" med_id">
									<div class="mb-3">
									...
									<div class="mb-3">
										...
										<select name="type_product_id" class="form_control" style="height: unset ! 
											important;">
											<option value="" disabled>ประเภทสินค้า</option>
											<?php foreach($result as $data):?>
												<option value="<?=$data['id']?>" <?php echo $row['type_product_id'] == $data['id'] ? 'selected' : '' ?>><?=$data['title']?></option>
											<?php endforeach;?>
										</select>
									</div>
									<div class="mb-3">
									    <label  class="form-label">ชื่อ</label>
									    <input type="text" class="form-control" name="med_name" value="<?php echo $row["med_name"]; ?>" >
									</div>
								    <div class="mb-3">
									    <label  class="form-label">รายละเอียด</label>
									    <input type="text" class="form-control" name="med_detail"  value="<?php echo $row["med_detail"]; ?>">
									</div>
                                    <div class="mb-3">
									    <label  class="form-label">ราคาซื้อ</label>
									    <input type="text" class="form-control" name="med_price" value="<?php echo $row["med_price"]; ?>">
									</div>
                                    <div class="mb-3">
									    <label  class="form-label">ราคาขาย</label>
									    <input type="text" class="form-control" name="med_cost" value="<?php echo $row["med_cost"]; ?>">
									</div>
